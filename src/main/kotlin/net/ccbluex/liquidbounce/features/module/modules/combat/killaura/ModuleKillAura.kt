...
object ModuleKillAura : ClientModule("KillAura", Category.COMBAT) {
    ...
    private fun updateTarget() {
        ...
        // SamePlayer logic: manage sticky target
        if (samePlayer) {
            ...
            if (sticky != null) {
                ...
                if (!sticky.isAlive || timePassed) {
                    ...
                    if (player.squaredBoxedDistanceTo(sticky) <= squaredMaxRange && sticky.shouldBeAttacked()) {
                        targetTracker.target = sticky
                        return
                    } else {
                        // If sticky target is not attackable or out of range,
                        // we still keep it as the target if samePlayer is enabled,
                        // but the attack function will not attack it.
                        targetTracker.target = sticky
                        return
                    }
                }
     ...
     */
     private fun raytraceEntityThroughEntities(range: Double, rotation: Rotation, targetEntity: Entity): Entity? {
        ...
        // Direct line check to target entity - ignoring other entities in between
        val targetBox = targetEntity.boundingBox
        val rayResult = targetBox.raycast(eyes, endPos)
        
        // If we can hit the target directly through line of sight
        if (rayResult.isPresent && targetEntity.shouldBeAttacked()) {
            return targetEntity
        }
