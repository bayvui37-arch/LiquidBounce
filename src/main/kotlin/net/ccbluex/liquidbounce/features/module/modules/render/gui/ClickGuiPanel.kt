...
import net.ccbluex.liquidbounce.features.module.modules.render.gui.GuiTextRenderer
import net.ccbluex.liquidbounce.render.engine.GuiElementBlurRenderer
import net.ccbluex.liquidbounce.render.engine.GuiBlurredRectRenderer
...
class ClickGuiPanel(
    ...
    private val useGlassEffect: Boolean = true
) {
    var x: Int
        get() = config.x
    ...
    fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        // Apply glassmorphism effect for panel background using shader from Download.zip
        if (useGlassEffect) {
            // Use the actual blurred_rect shader from Download.zip for true glassmorphism effect
            GuiBlurredRectRenderer.drawBlurredRect(x, y, width, actualHeight)
            
            // Draw header separator line
            context.fill(x, y + headerHeight - 1, x + width, y + headerHeight, 0x80FFFFFF.toInt()) // Lighter separator
            
            // Category name (adjusted for smaller tabs) - using SF Bold
            val categoryName = category.name.lowercase().replaceFirstChar { it.uppercase() }
            GuiTextRenderer.drawSfBoldText(context, categoryName, (x + 4).toFloat(), (y + 4).toFloat(), net.ccbluex.liquidbounce.render.engine.type.Color4b(255, 255, 255, 255))
            
            // Expand/collapse button (adjusted for smaller tabs)
            renderExpandButton(context, x + width - 12, y + 2)
        } else {
            // Fallback: draw basic rectangle without shader effect
            // Panel background
    ...
    private fun renderModule(renderData: ModuleRenderData) {
        ...
        val textColor4b = net.ccbluex.liquidbounce.render.engine.type.Color4b(
            (textColor shr 16) and 0xFF,
            (textColor shr 8) and 0xFF,
            textColor and 0xFF,
            255
        )
        GuiTextRenderer.drawSfBoldText(renderData.context, renderData.module.name, (renderData.moduleX + 4).toFloat(), (renderData.moduleY + 4).toFloat(), textColor4b)
